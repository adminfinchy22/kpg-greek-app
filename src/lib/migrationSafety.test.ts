...
describe('migration safety', () => {
  it('keeps noun_forms backfill aligned with untagged vocab policy', () => {
    ...
    const appliedDbBackfill = activeSql(migration('20260706110700_backfill_untagged_noun_forms.sql'))
  ...
  it('creates word_status as a security-invoker view for RLS', () => {
    for (const name of [
      ...
      '20260706110500_fix_word_status_security_invoker.sql',
    ]) {
  ...
  it('merges duplicate vocab progress before deleting the dropped row', () => {
    for (const name of [
      ...
      '20260706110600_preserve_duplicate_vocab_progress.sql',
    ]) {
      const sql = activeSql(migration(name))
